...
def pdf_to_mp3(pdf_path, mp3_path, lang='ko'):
    # PDF에서 텍스트를 추출한 후 MP3로 저장
    text = txt_to_text(pdf_path)
    if text:
        text = switch_txt(text)
        # print(text)
        # sp_txt = split_text(text)
        mp3_file_name = f"{mp3_path}.mp3"
        text_to_mp3(text, mp3_file_name, lang)
...
def split_text(text, max_length=4000, split_pattern=r'니다\.|습니다\.'):
    """
