...
def pdf_to_mp3(pdf_path, mp3_path, start_num=0,  lang='ko'):
    ...
    if text:
        ...
        for i, c_text in enumerate(sp_txt[start_num:],start=start_num):
            ...
            mp3_file_name = f"{mp3_path}_0{i}.mp3"
            text_to_mp3(en2kr(c_text), mp3_file_name, lang)
...
def split_text(text, max_length=4000, split_pattern=r'니다\.|습니다\.|었다\.|한다\.|였다\.'):
    """
...
def switch_txt(text):
    clean_text = re.sub(r'[<>《》=ㅅ;&ㅁㅇㄴ|+#$@*30]', '', text)
    return clean_text
